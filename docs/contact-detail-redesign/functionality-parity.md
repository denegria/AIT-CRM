...
| P08 | Editable Source selector; existing PATCH couples selected lead source and contact label when a lead exists, and contact-only source correction is supported without a lead. S: A, C, profile payload builder. B: 27 covers selected-lead editor only. | Record Contact → Correct contact source; Opportunity → Source → Correct opportunity source. | Preserve authorized correction, current/custom options and clearing for contact-only and selected-opportunity cases. P51 intentionally replaces coupled writes with independently scoped server contracts. Original submission events stay unchanged. No forced opportunity creation. | The prior read-only proposal would remove an existing attribution-correction capability. | S3; existing editor until then |
...
| source (existing combined control) | Existing coupled contract; revised contact-only and selected-opportunity-only scopes under P51 | Contact source in Edit contact; Opportunity source in its Source section. This adds a distinct scope/control without dropping either capability. |
...
- **UI:** Contact → Correct source opens the same contact-only editor at that field. Opportunity → Correct source opens the same opportunity editor's Source section. Explain "Updates this contact's source only" or "Updates this opportunity's source only". No nested scope-switch dialog or hidden second save. A draft belongs to its entity; unsaved drafts are never silently discarded.
