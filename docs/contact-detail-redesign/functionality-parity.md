...
The proposed detailed contact page must retain every capability inventoried below. The selected visual style remains; Record holds contact/opportunity facts and all applicable operational tabs retain their own records/actions. Focused editing uses scoped dialogs. Moving a control, consolidating duplicate entry points, or placing secondary fields in a disclosure is acceptable; removing an operation, field, permission distinction, history detail, or supported workflow is not.
...
**Status: 56 rows mapped; implementation and runtime parity not yet verified.** P01–P49 preserve the inventoried capabilities; P08 retains source correction while P51 deliberately improves its coupled-write behavior. P50 specifies continuity improvements and P51–P56 make the September 10 decisions explicit. These improvement rows are not claims of existing implementation. There are no intentional capability removals. This is a source-grounded inventory, not an end-to-end certification of every role, dataset, or successful mutation.

**Implementation is paused at the user's request.** Finish and review these artifacts; do not begin application edits or CRM writes without a subsequent instruction to resume. The September 10 instruction authorizes this documentation/board update and issue creation; a documentation staging push is not application-implementation approval.
...
| P08 | Editable Source selector; existing PATCH couples selected lead source and contact label when a lead exists, and contact-only source correction is supported without a lead. S: A, C, profile payload builder. B: 27 covers selected-lead editor only. | Header Edit contact → Contact source; Current inquiry → Edit → Source → Inquiry source. | Preserve authorized correction, current/custom options and clearing for contact-only and selected-opportunity cases. P51 intentionally replaces coupled writes with independently scoped server contracts. Original submission events stay unchanged. No forced opportunity creation. | The prior read-only proposal would remove an existing attribution-correction capability. | S3; existing editor until then |
...
| P14 | Start opportunity flow with division, initial status, assignment and reason. S: A, H. T: earlier route fixtures. | Quiet No active inquiry/closed state → secondary Start inquiry when authorized for actual new business. | Keep existing creation policy and fields; enforce duplicate-active protection. Do not make timeline inquiry events editable stand-ins for lead records. | Staff cannot reopen a relationship correctly after prior work ends. | S1, S3 |
...
| P28 | Current courses, course history selection and inspector; plural active enrollments supported. S: A, I. B: empty state only. T: earlier course fixtures. | Enrollments tab → actual current/planned/history rows and selected record. | Preserve all actual records, selection and current/history/planned distinctions. Use endpoint IDs rather than bootstrap summaries or row positions. | One course can hide another or an edit can target the wrong enrollment. | S1, S4 |
| P29 | Add Enrollment, default current/active workflow. S: A, I. | Enrollments tab → Add enrollment (opens existing contextual form). | Keep full course field ledger, validation and POST path, with explicit authorized relationship context. | Staff cannot record a new course. | S4; existing editor until then |
| P30 | Add Course History, default completed workflow. S: A, I. | Enrollments tab → Add history. | Preserve separate historical entry mode and relevant date/outcome fields; do not force history to masquerade as a current course. | Prior study history is lost or incorrectly counted as active. | S4; existing editor until then |
| P31 | Edit selected course record through PATCH. S: A, I. | Enrollments tab → selected record → Edit. | Read and save exact course ID; preserve all fields, server errors and refreshed selection. Existing values outside current options remain representable. | Corrections overwrite a different course or erase older values. | S4 |
| P32 | Complete active course shortcut. S: A, I. | Enrollments tab → active record → Complete. | Preserve completed preselection and editable terminal details before save; no automatic completion on opening. | Completion history becomes harder to record or changes without confirmation. | S4 |
| P33 | End active course shortcut with cancelled default and alternative terminal states. S: A, I. | Enrollments tab → active record → End enrollment. | Preserve cancelled/dropped/transferred/completed choices where policy permits, date/outcome behavior, and cancellation without mutation. | Drop/transfer/cancellation reasons become indistinguishable. | S4 |
...
| P35 | Receipt list with number/date/amount/status and Download PDF. S: A, L. B: empty list only. T: receipt format fixtures. | Receipts tab → saved receipt list → Download PDF. | Retain every visible receipt and download by actual financial record. Downloading an existing document must not create another payment. | Staff cannot find or reproduce a saved receipt. | S1, S4 |
| P36 | Generate Student Receipt, Enrolled lifecycle eligibility, complete payment form and Save & Download Receipt. S: A, L. B: 28, no submit. T. | Receipts tab → Generate student receipt (direct form). | Preserve financial permission, enrolled status rule, all five editable inputs and bound contact/division context. Save once, then generate PDF; a download failure offers retry of the saved receipt without another payment. | A core financial workflow is lost or creates duplicate payments. | S4; existing form until then |
| P37 | Linked work-order list and exact Open links; conditional related work can appear for USA too. S: A, B. | USA Work Orders tab when applicable; retain Signs Work Orders tab. | Do not assume USA has no work orders. Preserve title/number/due/status and exact record link under current visibility rules. | Existing related work becomes unreachable after removing tabs. | S1, S4 |
| P38 | Create Work Order entry with contact context and work-order permission. S: A. | Work Orders tab → existing contact-scoped Create work order route where authorized. | Preserve scoped route and permission; do not replace it with a decorative related-record heading. | Operators must restart work elsewhere and lose contact linkage. | S1, S4 |
...
| P43 | Signs linked people: name/role/primary marker/phone/email/notes and channel links. S: A, M. | Retain Signs linked Contacts tab; People is an optional clearer label, not a required Signs redesign. | Keep separate people and their exact links; do not flatten them into the account's primary identity. | Staff contact the wrong person at a client organization. | Retain; shared S1 regression gate |
...
| P50 | Audit found weak dirty-dismiss and return continuity; task URLs and drafts provide existing context to preserve. S/B: original F1/F10. | Shared editor host, URL-owned tab/filter/selected-resource state and preserved directory return. | Add reliable unsaved-change protection, focus restoration and directory/resource return state. Switching sections preserves fields; invalid/stale context fails safely. This is an improvement, not proven current parity. | Drafts are lost and operators must repeatedly reconstruct their place. | S1–S5 |

| P51 | Approved improvement: distinguish stored contact source from selected opportunity source; legacy contacts need no opportunity. Baseline S: A, C and contact-profile-patch.js. | Contact-only and opportunity-only source correction contracts. | Assert only the intended entity changes; authorized/denied/no-lead/closed/conflict/stale cases, custom/cleared values and unchanged original events. No historical guessing, bulk backfill or forced lead creation. Trace dependent ingestion/report consumers; leave old coupled form reachable until replacement passes. | A renamed editor can still overwrite unrelated attribution or exclude legacy contacts. | S3 |
| P52 | Approved additional fix: database-backed bootstrap falls back to seedData.SOURCES[index % length] when source is absent. S: bootstrap-data.js around 288; no live affected count measured. | Unknown/Not recorded for absent actual attribution; explicit contact fallback only where honestly labeled. | Missing lead/contact attribution never produces a sample channel; genuine stored source fields and provenance survive. Contact source is not mislabeled as the inquiry source. No data rewrite. | Fabricated attribution misleads operators and reporting. | S3 |
| P53 | Approved navigation contract: preserve each applicable production tab and avoid extra gateway navigation. S: A tab row around 1933; view-model conditions. | Record plus Activity, Conversations, Enrollments, Receipts/Financials, Work Orders and Signs linked People as applicable. | Each tab directly renders records and domain actions; preserve permissions and existing dedicated routes. One primary creation CTA; no gateway-only Manage/View replacement or duplicate Record collection. | The cleaner screen merely buries core workflows behind another page. | S1, S4 |
| P54 | Improvement: exact task behavior exists, but a deterministic next-task default was not specified. | Compact task band using actual authorized open follow-ups. | Valid incoming exact task first; invalid explicit context never substitutes. Default oldest overdue, earliest upcoming, then undated; ties createdAt then ID. Test completion/cancellation/denial eligibility and display time zone. | Next work is inconsistent or a different task is accidentally selected. | S2 |
| P55 | Improvement: direct owner/status correction without searching an editor section selector. | Opportunity → Change owner / Change status → same Ownership & status editor. | One action opens the relevant section; same draft/save path, eligible owners, required reasons, confirmations and stale handling. No duplicate inline mutation state. | Routine updates gain navigation steps or inconsistent save semantics. | S3 |
| P56 | Improvement: operational simplicity must be verified independently of capability existence. | Desktop-first operator walkthrough and populated tab/record fixtures. | Compare common jobs with current flow: completion success, wrong turns and unnecessary navigation. No extra mandatory navigation for common in-tab actions. Keep keyboard/zoom/basic responsive baseline without a mobile-first workstream. | All features can exist while the redesign remains confusing. | All |
...
### Profile editing — all 18 original inputs, with source split into explicit scopes
...
| source (existing combined control) | Existing coupled contract; revised contact-only and selected-opportunity-only scopes under P51 | Contact source in Edit contact; Inquiry source in Edit inquiry → Source (internal selected-opportunity scope). This adds a distinct scope/control without dropping either capability. |
...
### Source correction save contract — approved September 10 amendment

This intentionally replaces the prior requirement to preserve coupled contact/lead source writes. Preserve correction capability, not silent cross-entity side effects.

- **Contact source:** `contacts.sourceLabel`, available even without an opportunity. Label it Contact source, not guaranteed Original/First-touch source; historical coupled edits may have changed it. Correct contact source changes only this field under contact authorization. No opportunity is created, reopened or required merely to maintain the person.
- **Opportunity source:** selected lead source fields, corrected only for the captured exact opportunity ID under existing opportunity authorization/lifecycle/conflict policy. Correct opportunity source changes no contact label and never retargets a stale draft.
- **Evidence:** original submission/import events are read-only. Preserve `sourceType`/`sourceName` meanings, source references and Source Detail independently; do not overwrite an event, guess first-touch attribution or relink historical data.
- **Implementation:** add explicit scoped service/request contracts rather than sending ambiguous legacy `source` to the old coupled PATCH. Preserve current options, nonstandard current values and clearing. Map failures to the correct fields, keep drafts, and refresh only affected projections. Routine saves send changed fields only. Audit dependent ingestion/report read paths before changing semantics; no unrelated ingestion/provider rewrite.
- **UI:** Edit contact contains Contact source; Current inquiry → Edit contains Inquiry source in its Source section. Do not expose separate Correct source shortcuts in the read view. Explain "Updates this contact only" or "Updates this inquiry only. Contact source stays unchanged." No nested scope-switch dialog or hidden second save. A draft belongs to its entity; unsaved drafts are never silently discarded.
- **Unknown:** the current bootstrap's seed-source fallback is not evidence. Database-backed missing attribution displays Unknown/Not recorded. Show a genuine stored contact fallback only as Contact source, not as a proven opportunity origin. Do not rewrite data to populate guessed attribution.
- **Legacy and conflict:** no-opportunity is a normal supported state. Preserve existing permissions; contact-source correction follows contact access checks independently, while opportunity-source writes retain exact-record/conflict checks. Explicitly test that the new independent path does not broaden access to contact data through a conflicting or unauthorized opportunity.
- **Rollout:** existing correction remains reachable until these replacements pass P08/P51/P52. Do not label the new behavior implemented merely because a form exists; server effects must be verified.
...
3. Test admin, senior, regular coordinator and read-only behavior, including no-opportunity source correction; no-op and denied requests must not broaden permissions. Preserve Signs even though its redesign is outside this release.
...
5. Verify desktop 1440×1000 and compact 1280×800, 200% zoom, keyboard-only and focus return; add bounded narrow-width smoke for reachable controls/no overflow. Desktop workflows are the priority; no additional mobile concept program.
...
## Historical design gaps resolved, plus September 10 amendments

| Gap in the earlier concept/handoff | Observed evidence | Inferred consequence | Resolution |
|---|---|---|---|
| Acquisition source was made read-only in the proposed first release | Current Source selector and coupled PATCH are present; screenshot 27 | Would remove an existing correction workflow | Restore correction; September 10 P51 further separates contact/opportunity saves and covers no-opportunity contacts. |
...
## Historical verification record — September 7; remaining runtime gaps
...
## September 10 scope and verification

[MIS-408](https://linear.app/mission-control-v2/issue/MIS-408/ait-crm-detailed-contact-workspace-redesign-preserved-tabs-direct) owns the revised handoff. P51–P56 and revised destinations describe approved requirements, not application changes. All 56 tracker rows remain Not implemented / Not exercised. Documentation publication evidence belongs in the current release packet; historical tests and fictional boards do not prove runtime parity. No application implementation or CRM data write is authorized by this package alone.
