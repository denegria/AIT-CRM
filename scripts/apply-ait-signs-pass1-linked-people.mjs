...
const DEFAULT_INPUT = 'docs/mis-149-ait-signs-pass1-cluster-approval.json';
const DEFAULT_OUTPUT = 'docs/mis-149-ait-signs-pass1-linked-people-apply.json';
const DEFAULT_MARKDOWN = 'docs/mis-149-ait-signs-pass1-linked-people-apply.md';
const SOURCE_LABEL = 'ait_signs_pass1_latest_source_linked_people';
...
function buildApprovedPeople(packet) {
  ...
  for (const row of packet.approvalRows || []) {
    if (row.defaultRecommendation !== 'approve_listed_people') continue;
...
function primaryCandidateFor(contactId, existingPeopleByContact, plannedInserts) {
  const existing = existingPeopleByContact.get(contactId) || [];
  if (existing.some((person) => person.is_primary)) return false;
  return !plannedInserts.some((item) => item.contactId === contactId && item.isPrimary);
}
...
function buildPlan(approvedPeople, contactsByName, existingPeopleByContact, businessUnit) {
  ...
  for (const person of approvedPeople) {
    ...
    plannedInserts.push({
      ...
      email: null,
      notes: null,
      isPrimary: primaryCandidateFor(contact.id, existingPeopleByContact, plannedInserts),
      ...
      metadataJson: {
        ...
        rule: 'latest_pass1_source_reference',
      },
    });
  }
...
function renderMarkdown(report) {
  ...
  if (report.samples.skipped.length) {
    ...
    for (const item of report.samples.skipped) {
      lines.push(`- ${item.contactName}: ${item.name}; ${item.reason}`);
    }
  }

  if (report.samples.blocked.length) {
    lines.push('', '## Blocked', '');
    for (const item of report.samples.blocked) {
      lines.push(`- ${item.canonicalClient}: ${item.name}; ${item.reason}`);
    }
  }
