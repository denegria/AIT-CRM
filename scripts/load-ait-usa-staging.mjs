...
function proposalFor(record) {
  return record.proposedContactJson ||
...
function validateArtifact(payload) {
  ...
  if (payload.sourceType !== 'ait_usa_xlsx') {
    errors.push(`artifact sourceType must be ait_usa_xlsx; received ${payload.sourceType || 'missing'}`);
  }
  ...
  for (const record of payload.normalizedRecords) {
    ...
    if (record.recordType === 'activity_event') {
      if (proposal.eventType !== 'ait_usa.follow_up') {
        errors.push(`activity event ${key} must use ait_usa.follow_up`);
      }
      if (!proposal.message) {
        errors.push(`activity event ${key} is missing message text`);
      }
    }
  }
  ...
  return {
    ...
    summary: {
      ...
      duplicateActivityEventsSkipped: payload.counts?.duplicateActivityEventsSkipped || 0,
    },
  };
}
...
async function main() {
  ...
  console.log(`Loaded AIT USA staging batch ${result.batchId}`);
}
