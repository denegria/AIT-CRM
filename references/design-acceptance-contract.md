...
- **Closed-only empty state:** when the default open-task view has no rows but scoped tasks are closed, explain that there is no open work and point to Due → All Tasks. Do not imply a user-applied filter is hiding work.
...
- The follow-up-gap count uses the CRM workflow's actual active stages, excluding no-further-prospecting enrollment; an open follow-up task must bear a due date. A read-only staging breakdown showed the prior 1,811 count included 1,288 Retargeting and four Not Interested contacts, both terminal AIT USA stages. The corrected figure is coverage to review, not an employee-performance score.
...
# AIT USA Dashboard role and calendar addendum — 2026-09-25

- **Employee job:** regular coordinators open a truthful personal action summary; seniors see the same personal work plus a compact division-attention handoff. Counts must lead to the records they describe.
- **Reference mode:** inspiration from the accepted Contacts, Pipeline, Payments, Fulfillment, Tasks, and Team Monitor design system; no supplied image is a faithful implementation reference. Use the existing card, type, border, and spacing language.
- **Chosen interaction model:** a restrained row of four personal work signals (my overdue tasks, my due-today tasks, my new leads, my contacts missing a dated next follow-up), then a prominent full month calendar next to a focused task/action rail. The calendar is the dominant, useful work surface and must not be reduced to a miniature widget. Senior AIT USA additionally gets a compact Team needs attention band with an explicit Team Monitor handoff; do not embed a second employee roster.
- **Role and metric contract:** regular signals count only owned, authorized work. Seniors' personal signals use the same owner scope; team signals use the authorized selected division only. Task numbers derive from the loaded scoped task list, including visible zero. Contact counts use the same server directory filter semantics and date/owner/facet query as their destinations. Never label a workflow stage as due-date urgency. Senior header says Senior Coordinator, not Employee View.
- **States and permissions:** preserve all existing task completion/create behavior and server RBAC. The action rail shows overdue and due-today tasks first, capped with an explicit route to the full queue; a zero-work state is concise. Calendar month navigation, task event links, and calendar event content remain intact. Team metrics are read-only and link to existing authorized workspaces; no data writes, schema changes, auth changes, new transitions, or production promotion.
- **Visual and responsive invariants:** primary 1536×960 CSS pixels; regressions 1920×1080 and 390×844. Calendar keeps a generous month grid, preferably more horizontal room than before; the task rail does not stretch into a blank matching-height slab. At narrow widths stack rail and calendar without horizontal overflow, retain legible day cells/events, and preserve keyboard focus and reading order.
- **Content growth and evidence:** allow 0, 1, 5, and many urgent tasks; long titles must truncate or wrap within the rail. Prove senior/regular count-to-destination matches, blank-task KPI repair, senior team link, current calendar interaction, and no browser errors. Capture matched before/after screenshots for both roles at 1536×960; run focused metric/role tests, `npm run validate`, and read-only live staging QA. Staging contains little assigned regular work, so positive personal counts may need unit fixtures.

## Full-height task-rail follow-up — 2026-09-25

- **Owner correction:** Alvaro reviewed the staging screenshots and asked that the desktop task pane extend at least to the calendar's height. This supersedes the earlier compact-rail height invariant, not the large-calendar or role-scope decisions.
- **Interaction model:** keep urgent personal task rows at the top, the existing inline Add task flow at the bottom, and a small shortcut footer for the personal task queue and owned contacts. When no urgent tasks exist, center the concise clear-state message in the available work area; do not invent metrics or fill the rail with decorative panels. The senior-only Team needs attention band gains a direct, correctly filtered unassigned-task handoff alongside Team Monitor; no team work appears in the personal rail.
- **Visual direction:** at 1536×960 and 1920×1080 CSS pixels, the rail and calendar surfaces share a bottom edge. The rail may grow taller if real task content exceeds the calendar. At 390×844 CSS pixels, the rail returns to intrinsic height and stacks before the calendar, with no empty viewport-height slab or horizontal overflow.
- **Locked behavior:** existing task creation/completion, task-row follow-up logging, calendar navigation, metrics, permissions, and server routes stay unchanged. Only senior authorized users see the unassigned-task shortcut. Production and CRM business data remain untouched.
- **Evidence:** compare exact-commit senior and regular desktop renders against the prior staging screenshots; verify bottom-edge alignment, quick-link destinations, keyboard reachability, no browser errors, and mobile containment. Run focused UI/role checks, lint, and production build.

## Senior Dashboard focus follow-up — 2026-09-25

- **Owner feedback and job:** the senior Dashboard has too many numbers in one area: four personal KPI cards, three team counts, and repeated personal task urgency in the rail. A senior should see immediate team handoffs, their own urgent work, and the large calendar without a second Team Monitor.
- **Chosen interaction:** leave the regular coordinator's four personal cards unchanged. On the AIT USA senior view, remove that entire KPI row; the personal task rail already gives owned overdue/today work. Only nonzero personal new-lead or next-follow-up counts reappear as quiet, owner-scoped links inside My work. The senior team band becomes a concise **Team handoffs** summary: open unassigned tasks and overdue tasks assigned to someone other than the current user. These buckets must not double-count each other or personal tasks. Review unassigned retains its exact filtered Tasks handoff; Team Monitor remains the home for follow-up coverage and employee performance context.
- **Visual direction:** one compact top band, then the balanced full-height personal rail and generous month calendar. No row of seven competing numbers. The band states the actual scope (AIT USA) and gives a precise no-immediate-handoff state when both task buckets are zero; it must not imply that Team Monitor has no other coverage issues.
- **Locked behavior:** regular UI, admin route, task-row actions, Add task, calendar, contact metric filter semantics, business-unit scope, server RBAC, and production/data-write boundaries remain unchanged. Remove Dashboard-only team coverage COUNT requests when the count is no longer displayed; Team Monitor's own coverage metric is unchanged.
- **Evidence:** validate disjoint task buckets with own, other-owned, unassigned, closed, and cross-division fixtures. Confirm senior screenshot at 1536×960 is materially simpler, regular screenshot unchanged, Team Monitor and unassigned links work, no overflow/errors at 1536/1920/390, and local lint/build/focused tests pass.

## Senior Dashboard scan hierarchy correction — 2026-09-25

- **Owner correction:** the prior focus pass overcorrected. One prose-heavy handoff band does not read as a dashboard, and repeated “Plan ahead / Calendar” headings obscure rather than clarify the month view. Preserve the useful reduction in metric overlap without removing visual scanability.
- **Interaction and visual model:** show exactly three recognizable senior KPI cards across the top: **My overdue**, **Due today**, and **Unassigned**. Each number links to the exact authorized Tasks queue it counts. These are glanceable overview counters; the task pane beneath provides the actual rows and actions, so the overview/detail repetition is intentional. Put the scoped Team Monitor shortcut in the page header, not inside another explanatory panel. Do not restore the misleading total-team-overdue number or the 519 coverage count here; their context remains in Team Monitor.
- **Copy discipline:** no full-sentence Dashboard explanations. Senior task pane has one “My tasks” heading, short task/shortcut labels, and a concise empty state. The calendar month heading and controls are sufficient visually; retain an accessible Calendar label without displaying redundant “Plan ahead” or “Calendar” headings. Regular KPI cards, role permissions, task actions, and full-height desktop pane/calendar relationship remain intact.
- **Reference mode and viewports:** inspiration from the CRM's accepted KPI card language, not a faithful external visual. Primary 1536×960 CSS; regression 1920×1080 and 390×844. Three cards must remain legible on narrow screens without page overflow or crowding the month grid. Long counts should not break the row.
- **Evidence:** compare matched senior before/after staging screenshots, verify all three card destinations/counts, Team Monitor division scope, regular-role regression, accessibility label for calendar, focused tests, lint/build, and browser errors. No CRM business data write or production promotion.
