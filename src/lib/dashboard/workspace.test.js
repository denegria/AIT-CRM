...
test('team contact filters omit owner while retaining the matching coverage facet', () => {
  assert.deepEqual(dashboardContactFilter('teamNeedsFirstContact'), {
    leadDateScope: 'all', facet: 'needs_first_contact',
  });
  assert.equal(dashboardContactHref('teamNeedsNextFollowUp'), '/contacts?leadDateScope=all&facet=needs_next_follow_up');
});
...
test('task workspace separates own urgent work from authorized division attention', () => {
  ...
  assert.deepEqual(scope.teamOverdue.map((task) => task.id), ['mine-overdue', 'other-overdue']);
  assert.deepEqual(scope.teamUnassigned.map((task) => task.id), ['unassigned']);
});
