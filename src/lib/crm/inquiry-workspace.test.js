...
test('inquiry workspace projection whitelists exact CRM fields and defaults active before closed history', () => {
  ...
  assert.deepEqual(Object.keys(active).sort(), ['editable', 'id', 'isActive', 'lastActivityAt', 'openedAt', 'owner', 'placement', 'program', 'source', 'status']);
...
test('inquiry workspace projection keeps a placement action opaque and never serializes review evidence', () => {
  ...
  assert.deepEqual(item.placement, { state: 'In review', updatedAt: '2026-09-12T10:00:00.000Z', reviewPath: '/employee/placement-reviews?review=opaque-1' });
